...
test_expect_success '"checkout" removes deleted submodule' '
	git config -f .gitmodules submodule.submodule.path submodule &&
...
test_expect_success '"checkout" repopulates submodule' '
	submodule_creation_must_succeed delete_submodule base
'

test_expect_success '"checkout" repopulates submodule in existing directory' '
	git checkout delete_submodule &&
	mkdir submodule &&
	submodule_creation_must_succeed delete_submodule base
'

test_expect_success '"checkout" replaces submodule with files' '
	git checkout -f base &&
	...
	git checkout -f base &&
	git submodule update -f &&
	git checkout replace_submodule_with_dir &&
	test -d submodule &&
	! test -e submodule/.git &&
	test -f submodule/first.t &&
	test -f submodule/second.t
...
test_expect_success '"checkout" removes files and repopulates submodule' '
	submodule_creation_must_succeed replace_submodule_with_dir base
'

test_expect_failure '"checkout" replaces submodule with a file' '
	git checkout -f base &&
	...
	git checkout -f base &&
	git submodule update -f &&
	git checkout replace_submodule_with_file &&
	test -d submodule &&
	! test -e submodule/.git &&
	test -f submodule/first.t &&
	test -f submodule/second.t
...
test_expect_success '"checkout" removes the file and repopulates submodule' '
	submodule_creation_must_succeed replace_submodule_with_file base
'

test_expect_failure '"checkout" replaces submodule with a link' '
	git checkout -f base &&
	...
	git checkout -f base &&
	git submodule update -f &&
	git checkout replace_submodule_with_link &&
	test -d submodule &&
	! test -e submodule/.git &&
	test -f submodule/first.t &&
	test -f submodule/second.t
...
test_expect_success '"checkout" removes the link and repopulates submodule' '
...
test_expect_failure '"checkout" needs -f to update a modifed submodule commit' '
	(
		cd submodule &&
		git checkout HEAD^
	) &&
...
test_expect_failure '"checkout" needs -f to update modifed submodule content' '
...
test_expect_failure '"checkout" ignores modified submodule content that would not be changed' '
...
test_expect_failure '"checkout" does not care about untracked submodule content' '
...
test_expect_failure '"checkout" needs -f when submodule commit is not present (but does fail anyway)' '
